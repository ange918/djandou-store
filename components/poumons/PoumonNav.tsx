...
const navLinks = [
  ...
  { label: "Asthme", href: "/poumons/maladies/asthme" },
  { label: "Bronchite", href: "/poumons/maladies/bronchite" },
  { label: "Pneumonie", href: "/poumons/maladies/pneumonie" },
  { label: "Tuberculose", href: "/poumons/maladies/tuberculose" },
];

export default function PoumonNav() {
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            style={{ background: "rgba(5,8,16,0.97)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(56,189,248,0.1)" }}
          >
                ...
                return (
                  <motion.div key={link.href} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}>
                    <Link
                      ...
                      style={{ color: isActive ? "#38BDF8" : "#94A3B8", background: isActive ? "rgba(56,189,248,0.06)" : "transparent", fontFamily: "var(--font-poppins, Poppins, sans-serif)", fontWeight: 500, fontSize: "0.95rem" }}
                    >
                      {link.label}
                    </Link>
                  </motion.div>
