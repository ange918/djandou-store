      ...
      <div className="max-w-4xl mx-auto">
        ...
        <motion.div variants={fadeInUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="mb-12 p-8 rounded-2xl" style={{ background: "#0D1526", border: "1px solid rgba(255,255,255,0.06)" }}>
          <div className="flex items-center justify-between mb-6">
            ...
            <motion.span key={score} initial={{ scale: 1.3, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.3 }} style={{ fontFamily: "var(--font-unbounded, Unbounded, sans-serif)", fontWeight: 700, color: "#38BDF8", fontSize: "1.2rem" }}>
              {score}/{total}
            </motion.span>
          </div>
          ...
          {checklist.map((item, i) => {
            ...
            return (
              <motion.button
                ...
                style={{ background: isChecked ? "rgba(56,189,248,0.06)" : "#0D1526", border: `1px solid ${isChecked ? "rgba(56,189,248,0.3)" : "rgba(255,255,255,0.06)"}`, transition: "background 0.2s, border-color 0.2s" }}
              >
                <motion.div
                  className="flex-shrink-0 w-6 h-6 rounded-full border-2 flex items-center justify-center"
                  animate={{ scale: isChecked ? [1, 1.2, 1] : 1, borderColor: isChecked ? "#38BDF8" : "rgba(255,255,255,0.2)", background: isChecked ? "#38BDF8" : "rgba(0,0,0,0)" }}
