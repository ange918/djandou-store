    ...
    <>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <motion.div variants={fadeInUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="mb-16 p-10 rounded-2xl" style={{ background: "#0D1526", border: "1px solid rgba(255,255,255,0.06)" }}>
            <div className="flex items-center justify-between mb-4">
              ...
              <span style={{ fontFamily: "var(--font-unbounded, Unbounded, sans-serif)", fontWeight: 700, color: cfg.color, fontSize: "1.1rem" }}>{pronostic}</span>
          ...
          <motion.div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-16" variants={container} initial="hidden" whileInView="visible" viewport={{ once: true }}>
            {complications.map((c, i) => (
              <motion.div key={i} variants={fadeInUp} whileHover={{ scale: 1.04, y: -3, transition: { duration: 0.2 } }} className="p-5 rounded-xl"
                style={{ background: c.gravite === "rouge" ? "rgba(239,68,68,0.06)" : "rgba(249,115,22,0.06)", border: `1px solid ${c.gravite === "rouge" ? "rgba(239,68,68,0.2)" : "rgba(249,115,22,0.2)"}` }}
              >
                <p style={{ fontFamily: "var(--font-poppins, Poppins, sans-serif)", fontWeight: 500, color: c.gravite === "rouge" ? "#EF4444" : "#F97316", fontSize: "0.9rem" }}>{c.nom}</p>
              </motion.div>
            ))}
          </motion.div>

          <div className="mb-16">
            <motion.h3 className="text-xl font-bold mb-10" initial={{ opacity: 0, y: 20, filter: "blur(4px)" }} whileInView={{ opacity: 1, y: 0, filter: "blur(0px)" }} viewport={{ once: true }} transition={{ duration: 0.6 }}
            ...
            <div className="relative flex flex-col gap-0">
              ...
              {timeline.map((t, i) => (
                <motion.div key={i} initial={{ opacity: 0, x: -30, filter: "blur(4px)" }} whileInView={{ opacity: 1, x: 0, filter: "blur(0px)" }} viewport={{ once: true }} transition={{ delay: i * 0.1, duration: 0.6, ease: "easeOut" as const }} className="flex gap-6 items-start pb-8 last:pb-0">
                  <div className="relative flex-shrink-0 z-10 w-12 h-12 rounded-full border-2 flex items-center justify-center" style={{ background: "#050810", borderColor: "#38BDF8" }}>
                    <span style={{ fontFamily: "var(--font-unbounded, Unbounded, sans-serif)", fontWeight: 900, color: "#38BDF8", fontSize: "0.65rem" }}>{t.time}</span>
                  </div>
                  <div className="flex-1 pt-2">
                    <p style={{ fontFamily: "var(--font-poppins, Poppins, sans-serif)", fontWeight: 300, color: "#94A3B8", lineHeight: 1.7, fontSize: "0.92rem" }}>{t.event}</p>
                  </div>
                </motion.div>
              ))}
            </div>
      ...
      <section className="py-24 px-6" style={{ background: "#080D18" }}>
        <div className="max-w-5xl mx-auto">
          ...
          <motion.div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12" variants={container} initial="hidden" whileInView="visible" viewport={{ once: true }}>
            {autresMaladies.map((m, i) => (
              <motion.div key={i} variants={fadeInUp}>
                <Link href={m.href}>
                  <motion.div whileHover={{ y: -6, scale: 1.03, borderColor: "rgba(56,189,248,0.3)" }} transition={{ duration: 0.2 }} className="p-6 rounded-2xl h-full" style={{ background: "#0D1526", border: "1px solid rgba(255,255,255,0.06)" }}>
                    <h4 className="font-bold mb-3" style={{ fontFamily: "var(--font-unbounded, Unbounded, sans-serif)", fontWeight: 700, color: "#F8FAFC", fontSize: "0.85rem" }}>{m.titre}</h4>
                    <p style={{ fontFamily: "var(--font-poppins, Poppins, sans-serif)", fontWeight: 300, color: "#64748B", fontSize: "0.85rem", lineHeight: 1.7 }}>{m.desc}</p>
                    <p className="mt-4 text-sm font-medium" style={{ color: "#38BDF8", fontFamily: "var(--font-poppins, Poppins, sans-serif)" }}>Découvrir →</p>
                  </motion.div>
                </Link>
              </motion.div>
            ))}
          ...
          <motion.div className="flex flex-wrap gap-4 justify-center" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.97 }}>
              <Link href="/poumons" className="inline-flex items-center gap-2 px-8 py-4 rounded-full text-sm font-medium transition-all duration-200 hover:bg-white/5" style={{ border: "1px solid rgba(56,189,248,0.3)", color: "#38BDF8", fontFamily: "var(--font-poppins, Poppins, sans-serif)" }}>
                ← Retour au volet Poumons
              </Link>
            </motion.div>
          </motion.div>
        </div>
