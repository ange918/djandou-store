...
import { BiHeart, BiMenu, BiX, BiArrowBack } from "react-icons/bi";
      ...
      >
        <div className="max-w-7xl mx-auto px-6 w-full flex items-center justify-between gap-8">
          <Link href="/coeur" className="flex items-center gap-2.5 group flex-shrink-0">
            <BiHeart size={24} className="transition-transform duration-200 group-hover:scale-110" style={{ color: "#EF4444" }} />
            <span style={{ fontFamily: "var(--font-unbounded, Unbounded, sans-serif)", fontWeight: 700, fontSize: "0.95rem", color: "#F8FAFC" }}>
              MediSens <span style={{ color: "#EF4444" }}>/</span> <span style={{ color: "#EF4444" }}>Cœur</span>
            </span>
          </Link>
                ...
                return (
                  <motion.div key={link.href} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}>
                    <Link
                      ...
                      style={{ color: isActive ? "#EF4444" : "#94A3B8", background: isActive ? "rgba(239,68,68,0.06)" : "transparent", fontFamily: "var(--font-poppins, Poppins, sans-serif)", fontWeight: 500, fontSize: "0.95rem" }}
                    >
                      {link.label}
                    </Link>
                  </motion.div>
