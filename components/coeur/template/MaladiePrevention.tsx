      ...
      <div className="max-w-4xl mx-auto">
        ...
        <motion.div variants={fadeInUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="mb-12 p-8 rounded-2xl" style={{ background: "#0D1526", border: "1px solid rgba(255,255,255,0.06)" }}>
          <div className="flex items-center justify-between mb-6">
            <span style={{ fontFamily: "var(--font-poppins, Poppins, sans-serif)", fontWeight: 500, color: "#F8FAFC", fontSize: "0.95rem" }}>Votre score de prévention</span>
            <motion.span key={score} initial={{ scale: 1.3, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.3 }} style={{ fontFamily: "var(--font-unbounded, Unbounded, sans-serif)", fontWeight: 700, color: "#EF4444", fontSize: "1.2rem" }}>
              {score}/{total}
            </motion.span>
          </div>
          <div className="w-full rounded-full h-3 overflow-hidden" style={{ background: "rgba(255,255,255,0.06)" }}>
            <motion.div className="h-3 rounded-full" style={{ background: "linear-gradient(90deg, #EF4444, #B91C1C)" }} initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ duration: 0.5, ease: "easeOut" as const }} />
          </div>
          ...
          {checklist.map((item, i) => {
            ...
            return (
              <motion.button
                ...
                style={{ background: isChecked ? "rgba(239,68,68,0.06)" : "#0D1526", border: `1px solid ${isChecked ? "rgba(239,68,68,0.3)" : "rgba(255,255,255,0.06)"}`, transition: "background 0.2s, border-color 0.2s" }}
              >
                <motion.div
                  className="flex-shrink-0 w-6 h-6 rounded-full border-2 flex items-center justify-center"
                  animate={{ scale: isChecked ? [1, 1.2, 1] : 1, borderColor: isChecked ? "#EF4444" : "rgba(255,255,255,0.2)", background: isChecked ? "#EF4444" : "rgba(0,0,0,0)" }}
                  transition={{ duration: 0.25 }}
                >
                  <AnimatePresence>
                    {isChecked && (
                      <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }} className="text-white text-xs font-bold">✓</motion.span>
                    )}
                  </AnimatePresence>
                </motion.div>
                <span style={{ fontFamily: "var(--font-poppins, Poppins, sans-serif)", fontWeight: isChecked ? 400 : 300, fontSize: "0.95rem", color: isChecked ? "#F8FAFC" : "#94A3B8", transition: "color 0.2s" }}>
                  {item}
                </span>
              </motion.button>
            );
        ...
        {populationsRisque.length > 0 && (
          ...
          >
            <div className="flex items-center gap-3 mb-5">
              <BiShieldX size={24} style={{ color: "#EF4444", flexShrink: 0 }} />
            ...
            <ul className="flex flex-col gap-2">
              {populationsRisque.map((p, i) => (
                <motion.li key={i} initial={{ opacity: 0, x: -15 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.06, duration: 0.4 }} className="flex items-start gap-3">
                  <span style={{ color: "#EF4444", flexShrink: 0, marginTop: "2px" }}>•</span>
                  <span style={{ fontFamily: "var(--font-poppins, Poppins, sans-serif)", fontWeight: 300, fontSize: "0.92rem", color: "#94A3B8", lineHeight: 1.7 }}>{p}</span>
                </motion.li>
              ))}
            </ul>
          </motion.div>
