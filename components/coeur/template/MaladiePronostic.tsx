        ...
        <div className="max-w-5xl mx-auto">
          ...
          <motion.div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12" variants={container} initial="hidden" whileInView="visible" viewport={{ once: true }}>
            {autresMaladies.map((m, i) => (
              <motion.div key={i} variants={fadeInUp}>
                <Link href={m.href}>
                  <motion.div whileHover={{ y: -6, scale: 1.03, borderColor: "rgba(239,68,68,0.3)" }} transition={{ duration: 0.2 }} className="p-6 rounded-2xl h-full" style={{ background: "#0D1526", border: "1px solid rgba(255,255,255,0.06)" }}>
                    ...
                    <p className="mt-4 text-sm font-medium" style={{ color: "#EF4444", fontFamily: "var(--font-poppins, Poppins, sans-serif)" }}>Découvrir →</p>
                  </motion.div>
                </Link>
              </motion.div>
            ))}
          ...
          <motion.div className="flex flex-wrap gap-4 justify-center" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.97 }}>
              <Link href="/coeur" className="inline-flex items-center gap-2 px-8 py-4 rounded-full text-sm font-medium transition-all duration-200 hover:bg-white/5" style={{ border: "1px solid rgba(239,68,68,0.3)", color: "#EF4444", fontFamily: "var(--font-poppins, Poppins, sans-serif)" }}>
