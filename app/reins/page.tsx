          ...
          >
            {stats.map((stat, i) => (
              ...
              >
                ...
                <p className="text-sm max-w-[180px] mx-auto" style={{ color: "#64748B", fontWeight: 300, lineHeight: 1.7 }}>
                  {stat.label}
                </p>
              </motion.div>
            ))}
            ...
            {maladies.map((m, i) => (
              <GlowCard key={i} href={m.href}>
                <div className="flex items-start gap-4 mb-5">
                  <div className="flex-shrink-0 w-12 h-12 rounded-xl flex items-center justify-center" style={{ background: "rgba(0,201,255,0.08)", color: "#00C9FF" }}>
                    {m.icon}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2 mb-1">
                      <h3 className="text-base sm:text-lg font-bold leading-tight" style={{ fontFamily: "var(--font-unbounded, Unbounded, sans-serif)", color: "#F8FAFC", fontWeight: 700 }}>
                        {m.titre}
                      </h3>
                      <span className="text-xs font-medium px-2.5 py-1 rounded-full flex-shrink-0" style={{ color: m.badgeColor, background: `${m.badgeColor}15`, border: `1px solid ${m.badgeColor}33` }}>
                        {m.badge}
                      </span>
                    </div>
                  </div>
      ...
      <section className="py-16 md:py-32 px-6" style={{ background: "#080D18" }}>
        <div className="max-w-2xl mx-auto text-center">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}>
            <span className="inline-block mb-6 px-4 py-1.5 rounded-full text-xs font-medium tracking-[0.2em] uppercase" style={{ color: "#00C9FF", background: "rgba(0,201,255,0.08)", border: "1px solid rgba(0,201,255,0.2)", fontFamily: "var(--font-poppins, Poppins, sans-serif)" }}>
              DÉPISTAGE
            </span>
            <h2 className="mb-6" style={{ fontFamily: "var(--font-unbounded, Unbounded, sans-serif)", fontWeight: 700, fontSize: "clamp(1.75rem, 4vw, 2.75rem)", lineHeight: 1.2, color: "#F8FAFC" }}>
              Êtes-vous à risque ?
            </h2>
            <p className="mb-12 max-w-xl mx-auto" style={{ color: "#64748B", fontWeight: 300, lineHeight: 1.8 }}>
              En 10 questions, évaluez votre profil de risque de développer une maladie rénale. Un outil d&apos;orientation, pas de diagnostic médical.
            </p>
            <Link href="/outils" className="inline-flex items-center gap-3 px-8 sm:px-10 py-4 rounded-full font-medium text-white transition-all duration-200 hover:scale-105" style={{ background: "linear-gradient(135deg, #00C9FF, #0066FF)", fontFamily: "var(--font-poppins, Poppins, sans-serif)", fontWeight: 500, fontSize: "1rem" }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-0">
            {gestes.map((g, i) => (
              ...
              >
                ...
                <h3 className="text-lg sm:text-xl font-bold mb-6" style={{ fontFamily: "var(--font-unbounded, Unbounded, sans-serif)", color: "#F8FAFC" }}>
                  {g.titre}
                </h3>
                <p className="max-w-xs mx-auto" style={{ color: "#64748B", fontWeight: 300, lineHeight: 1.8, fontSize: "0.925rem" }}>
                  {g.desc}
                </p>
              </motion.div>
            ))}
